document.addEventListener("DOMContentLoaded", () => {
  // === IMAGE SLIDER ===
  const slides = document.querySelectorAll(".slides img");
  let slideIndex = 0;
  let intervalID = null;

  if (slides.length > 0) {
    slides.forEach(slide => (slide.style.display = "none"));
    showSlide(slideIndex);
    intervalID = setInterval(() => changeSlide(1), 5000);
  }

  function showSlide(index) {
    slides.forEach(slide => (slide.style.display = "none"));
    if (index >= slides.length) index = 0;
    if (index < 0) index = slides.length - 1;
    slides[index].style.display = "block";
    slideIndex = index;
  }

  function changeSlide(step) {
    let newIndex = slideIndex + step;
    if (newIndex >= slides.length) newIndex = 0;
    if (newIndex < 0) newIndex = slides.length - 1;
    showSlide(newIndex);
  }

  window.nextSlide = function () {
    clearInterval(intervalID);
    changeSlide(1);
    intervalID = setInterval(() => changeSlide(1), 5000);
  };

  window.prevSlide = function () {
    clearInterval(intervalID);
    changeSlide(-1);
    intervalID = setInterval(() => changeSlide(1), 5000);
  };

  // === SHUFFLE GOALS LIST ===
  const goalList = document.getElementById("goals-list");
  if (goalList) {
    window.shuffleGoals = function () {
      const items = Array.from(goalList.children);
      for (let i = items.length - 1; i > 0; i--) {
        const j = Math.floor(Math.random() * (i + 1));
        [items[i], items[j]] = [items[j], items[i]];
      }
      items.forEach(item => goalList.appendChild(item));
    };
  }

  // === EMAILJS CONTACT FORM ===
  const contactForm = document.getElementById("contactForm");
  const result = document.getElementById("result");
  let cooldown = false;
  
  if (contactForm && result) {
    emailjs.init("fJ4bUNEbuwI76kNto");
  
    contactForm.addEventListener("submit", function (e) {
      e.preventDefault();


  
      const lastSend = Number(sessionStorage.getItem("lastSend") || 0);
      const now = Date.now();
      const cooldownTime = 60 * 1000; 
  
      if (now - lastSend < cooldownTime) {
        const secondsLeft = Math.ceil((cooldownTime - (now - lastSend)) / 1000);
      
        let countdown = secondsLeft;
      
        // Start live countdown
        const countdownInterval = setInterval(() => {
          result.textContent = `⏳ Please wait ${countdown}s before sending another message.`;
          result.style.color = "orange";
          countdown--;
      
          if (countdown <= 0) {
            clearInterval(countdownInterval);
            result.textContent = "You can send another message now.";
            result.style.color = "green";
      
            setTimeout(() => {
              result.textContent = "If you would like to get in touch with me, you can reach me via email or socials";
              result.style.color = "";
            }, 3000);
          }
        }, 1000);
      
        return;
      }

      
  
      // Honeypot
      const honeypot = contactForm.querySelector("input[name='_honey']");
      if (honeypot && honeypot.value) {
        result.textContent = "❌ Spam detected. Form submission blocked.";
        result.style.color = "red";
        return;
      }
  
      // Email value
      const emailInput = contactForm.querySelector("input[name='from_email']");
      const email = emailInput.value.trim();
  
      // Regex check
      const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
      if (!emailRegex.test(email)) {
        result.textContent = "❌ Please enter a valid email address.";
        result.style.color = "red";
        return;
      }
  
      // Manual domain block
      const blockedDomains = [
        "tempmail.com",
        "10minutemail.com",
        "yopmail.com",
        "mailinator.com",
        "fakemail.net",
        "dispostable.com"
      ];
      const domain = email.split("@")[1];
      if (blockedDomains.includes(domain)) {
        result.textContent = "❌ Emails from this domain are not allowed.";
        result.style.color = "red";
        return;
      }
  
      // Kickbox disposable email check
      fetch(`https://open.kickbox.com/v1/disposable/${encodeURIComponent(email)}`)
        .then(res => res.json())
        .then(data => {
          if (data.disposable) {
            result.textContent = "❌ Temporary or fake email addresses are not allowed.";
            result.style.color = "red";
            return;
          }
  
          // SEND with EmailJS
          emailjs.sendForm("service_ejb45tm", "template_gpy5l1d", contactForm)
            .then(() => {
              result.textContent = "✅ Message sent successfully!";
              result.style.color = "green";
              contactForm.reset();
  
              sessionStorage.setItem("lastSend", Date.now());
  
              // Reset message
              setTimeout(() => {
                result.textContent = "If you would like to get in touch with me, you can reach me via email or socials";
                result.style.color = "";
              }, 5000);
            })
            .catch(error => {
              console.error("❌ FAILED:", error);
              result.textContent = "❌ Something went wrong. Please try again.";
              result.style.color = "red";
  
              setTimeout(() => {
                result.textContent = "If you would like to get in touch with me, you can reach me via email or socials";
                result.style.color = "";
              }, 5000);
            });
        })
        .catch(error => {
          console.error("Kickbox error:", error);
          result.textContent = "❌ Could not verify email. Please try again.";
          result.style.color = "red";
        });
    });
  }
  

  // === DARK/LIGHT TOGGLE ===
  const toggleBtn = document.getElementById("themeToggle");
  const themeIcon = document.getElementById("themeIcon");

  if (toggleBtn && themeIcon) {
    toggleBtn.addEventListener("click", () => {
      document.body.classList.toggle("dark-mode");
      const isDark = document.body.classList.contains("dark-mode");
      themeIcon.src = isDark ? "ASSETS/sun.png" : "ASSETS/moon.png";
      localStorage.setItem("theme", isDark ? "dark" : "light");
    });

    // Load saved theme
    const savedTheme = localStorage.getItem("theme");
    if (savedTheme === "dark") {
      document.body.classList.add("dark-mode");
      themeIcon.src = "ASSETS/sun.png";
    } else {
      themeIcon.src = "ASSETS/moon.png";
    }
  }
});

// FETCH JSON PROJECTS

const projects = [
  {
    "title": "Buddo's Portfolio",
    "date": "9/6/2025",
    "description": "The one you are looking at!",
    "image": "ASSETS/placeholder.png",
    "skills": ["HTML", "CSS", "JavaScript", "JSON"],
    "link": "https://lakbud.github.io/Portfolio-Project"
  }
];



document.addEventListener("DOMContentLoaded", () => {
  fetch("projects.json")
    .then((res) => res.json())
    .then((projects) => {
      const projectList = document.getElementById("project-list");

      projects.forEach((project) => {
        const article = document.createElement("article");
        article.classList.add("project-container");

        article.innerHTML = `
          <img src="${project.image}" alt="${project.title}" class="project-img" />
          <div class="project-txt">
            <h3 class="project-h3">${project.title}</h3>
            <p class="project-p">${project.description}</p>
            <div class="skills-container">
              ${project.skills.map(skill => `<span class="skills">${skill}</span>`).join("")}
            </div>
            <p class="project-date">${project.date}</p>
            <a href="${project.link}" target="_blank">View Project</a>
          </div>
        `;

        projectList.appendChild(article);
      });
    })
    .catch((error) => console.error("Failed to load projects:", error));
});


